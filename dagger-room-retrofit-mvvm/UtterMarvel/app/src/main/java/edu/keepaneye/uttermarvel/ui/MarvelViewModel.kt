...
class MarvelViewModel(
    ...
    fun searchForCharacters(nameStartsWith: String) = viewModelScope.launch {
        charactersList.postValue(Resource.Loading())
        val response = marvelRepository.searchForCharacters(
            offset = searchCharactersListOffset,
    ...
    fun searchForComics(titleStartsWith: String) = viewModelScope.launch {
        comicsList.postValue(Resource.Loading())
        val response = marvelRepository.searchForComics(
            offset = searchComicsListOffset,
    ...
    fun saveCharacter(character: CharacterItem) = viewModelScope.launch {
        marvelRepository.upsertCharacter(character = character)
    }

    fun getSavedCharacters() = marvelRepository.getSavedCharacters()

    fun deleteCharacter(character: CharacterItem) = viewModelScope.launch {
        marvelRepository.deleteCharacter(character = character)
    }

    fun saveComics(comics: ComicsItem) = viewModelScope.launch {
        marvelRepository.upsertComics(comics = comics)
    }

    fun getSavedComics() = marvelRepository.getSavedComics()

    fun deleteComics(comics: ComicsItem) = viewModelScope.launch {
        marvelRepository.deleteComics(comics = comics)
    }
}